...
import type { UserCrop, CropEvent } from '@/lib/userCrops';
...
function loadUserCrops(): UserCrop[] {
...
function saveUserCrops(crops: UserCrop[]) {
  if (typeof window === 'undefined') {
    return;
  }
...
function findCropById(id: number): UserCrop | undefined {
  return loadUserCrops().find((item) => item.id === id);
}

function updateCropInStorage(updatedCrop: UserCrop) {
  const crops = loadUserCrops();
  const nextCrops = crops.map((item) => (item.id === updatedCrop.id ? updatedCrop : item));

  if (!nextCrops.some((item) => item.id === updatedCrop.id)) {
    nextCrops.push(updatedCrop);
  }

  saveUserCrops(nextCrops);
}
...
export default function CropPage() {
  ...
  const parsedCropId = Number(cropIdFromParams);

  const [crop, setCrop] = useState<UserCrop | null>(null);
  ...
  useEffect(() => {
    if (!cropIdFromParams || Number.isNaN(parsedCropId)) {
      setCrop(null);
      return;
    }

    setCrop(findCropById(parsedCropId) ?? null);
  }, [cropIdFromParams, parsedCropId]);
  ...
  const handleDeleteCrop = () => {
    ...
    const crops = loadUserCrops();
    const updatedCrops = crops.filter((item) => item.id !== crop.id);
    ...
    return (
      <main className="min-h-screen bg-beige px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl rounded-3xl border border-red-200 bg-white p-8 shadow-sm">
          ...
          <p className="mt-3 text-slate-600">Controlla l&apos;ID della coltura o torna alla dashboard.</p>
          <div className="mt-6">
