    ...
    <section className="rounded-[2rem] border border-slate-200 bg-white p-8 md:p-10 shadow-xl">
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-2">
        {visibleNews.map((item) => (
          <article key={item.id} className="group rounded-[2rem] border border-slate-200 bg-slate-50 p-6 min-h-[260px] cursor-pointer transition-all duration-200 hover:-translate-y-1 hover:border-olive/20 hover:bg-white hover:shadow-lg">
            <div className="flex items-start gap-4">
              <div className="flex h-14 w-14 items-center justify-center rounded-3xl bg-olive/10 text-2xl">
                {item.icon}
              </div>
              <div>
                <p className="text-base md:text-lg font-semibold text-slate-900">{item.title}</p>
